...
def plot_additions_distribution(df):
    ...
    ax.hist(df['total_additions'].clip(upper=1000), bins=50, edgecolor='black', alpha=0.7)
    ax.set_xlabel('Lines Added per PR')
    ax.set_ylabel('Number of PRs')
    ax.set_title('Distribution of Additions per PR\n(Clipped at 1000 lines for visibility)')
    ax.axvline(df['total_additions'].median(), color='red', linestyle='--', 
...
def plot_deletions_distribution(df):
    ...
    ax.hist(df['total_deletions'].clip(upper=500), bins=50, edgecolor='black', alpha=0.7, color='orange')
    ax.set_xlabel('Lines Deleted per PR')
    ax.set_ylabel('Number of PRs')
    ax.set_title('Distribution of Deletions per PR\n(Clipped at 500 lines for visibility)')
    ax.axvline(df['total_deletions'].median(), color='red', linestyle='--',
...
def plot_files_distribution(df):
    ...
    ax.hist(files_clipped, bins=50, edgecolor='black', alpha=0.7, color='green')
    ax.set_xlabel('Files Touched per PR')
    ax.set_ylabel('Number of PRs')
    ax.set_title('Distribution of Files Touched per PR\n(Clipped at 50 files for visibility)')
    ax.axvline(df['files_touched'].median(), color='red', linestyle='--',
...
def plot_size_distribution(df):
    ...
    bars = ax.bar(size_counts.index, size_counts.values, edgecolor='black', alpha=0.7)
    
    # Add percentages on bars
    for i, (cat, count) in enumerate(size_counts.items()):
        pct = (count / len(df)) * 100
        ax.text(i, count + 500, f'{pct:.1f}%', ha='center', va='bottom', fontweight='bold')
    
    ax.set_xlabel('PR Size Category')
    ax.set_ylabel('Number of PRs')
    ax.set_title('Distribution of PR Sizes')
...
def plot_combined_overview(df):
    ...
    ax1.hist(df['total_additions'].clip(upper=1000), bins=30, edgecolor='black', alpha=0.7)
    ax1.set_xlabel('Lines Added')
    ax1.set_ylabel('Count')
    ax1.set_title('Additions Distribution')
    ax1.axvline(df['total_additions'].median(), color='red', linestyle='--', linewidth=2)
    
    # 2. Deletions histogram
    ax2 = fig.add_subplot(gs[0, 1])
    ax2.hist(df['total_deletions'].clip(upper=500), bins=30, edgecolor='black', alpha=0.7, color='orange')
    ax2.set_xlabel('Lines Deleted')
    ax2.set_ylabel('Count')
    ax2.set_title('Deletions Distribution')
    ax2.axvline(df['total_deletions'].median(), color='red', linestyle='--', linewidth=2)
    
    # 3. Files histogram
    ax3 = fig.add_subplot(gs[0, 2])
    ax3.hist(df['files_touched'].clip(upper=50), bins=30, edgecolor='black', alpha=0.7, color='green')
    ax3.set_xlabel('Files Touched')
    ax3.set_ylabel('Count')
    ax3.set_title('Files Distribution')
    ax3.axvline(df['files_touched'].median(), color='red', linestyle='--', linewidth=2)
    
    # Row 2: Size category bar chart (spans all columns)
    ax4 = fig.add_subplot(gs[1, :])
    size_order = ['XS', 'S', 'M', 'L', 'XL']
    size_counts = df['size_category'].value_counts().reindex(size_order)
    bars = ax4.bar(size_counts.index, size_counts.values, edgecolor='black', alpha=0.7)
    for i, (cat, count) in enumerate(size_counts.items()):
        pct = (count / len(df)) * 100
        ax4.text(i, count, f'{pct:.1f}%', ha='center', va='bottom', fontweight='bold')
    ax4.set_xlabel('PR Size Category')
    ax4.set_ylabel('Number of PRs')
    ax4.set_title('PR Size Distribution')
    ...
    fig.suptitle('Agentic-PR Metrics Overview', fontsize=16, fontweight='bold', y=0.995)
